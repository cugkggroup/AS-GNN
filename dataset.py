...
def get_tg_dataset(args, dataset_name, use_cache=True, remove_feature=False):
    ...
    if use_cache and ((os.path.isfile(f2_name) and args.task=='link') or (os.path.isfile(f1_name) and args.task!='link')):
        ...
        for i, data in enumerate(dataset):
            ...
            if args.task=='link':
                data.dists = torch.from_numpy(dists_removed_list[i]).float()#Creates a Tensor from a numpy.ndarray
...
def Graph_load_batch(min_num_nodes = 20, max_num_nodes = 1000, name = 'ENZYMES',node_attributes = True,graph_labels=True):
    ...
    G = nx.Graph()
    # load data
    path = '/content/P-GNN-Google/P-GNN-master/data/'+name+'/'
...
def load_graphs(dataset_str):
    ...
    if dataset_str == 'grid':
        ...
        with open('/content/P-GNN-Google/P-GNN-master/data/email.txt', 'rb') as f:
            graph = nx.read_edgelist(f)

        label_all = np.loadtxt('/content/P-GNN-Google/P-GNN-master/data/email_labels.txt')
