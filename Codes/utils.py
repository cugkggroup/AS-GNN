...
def get_random_anchorset(n, filename="grid", subgraph_id = 0, c=0.5):
...
def preselect_anchor(data, layer_num=1, anchor_num=32, anchor_size_num=4, filename="grid", subgraph_id=0, device='cpu'):

    data.anchor_size_num = anchor_size_num
    data.anchor_set = []
    #suit with node size
    anchor_num = data.num_nodes//10
    anchor_num_per_size = anchor_num//anchor_size_num
    for i in range(anchor_size_num):
        anchor_size = 2**(i+1)-1
        anchors = np.random.choice(data.num_nodes, size=(layer_num,anchor_num_per_size,anchor_size), replace=False)#True
        data.anchor_set.append(anchors)
    data.anchor_set_indicator = np.zeros((layer_num, anchor_num, data.num_nodes), dtype=int)

    anchorset_id = get_random_anchorset(data.num_nodes, filename, subgraph_id, c=1)
